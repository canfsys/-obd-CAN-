...
```mermaid
graph TD
    HW["PCB 硬件设计<br/>(hardware/)"] -->|驱动| FW["ESP32-S3 车载终端固件<br/>(esp32/)"]
    FW <-->|"UART 串口协议"| IOT["4G 物联网模块<br/>(iot/) 银尔达 M100PG-DTU"]
    IOT -->|"HTTP POST (4G网络)"| CLOUD["云函数 upload<br/>(云数据库存储)"]
    CLOUD <-->|"轮询 15 秒"| MP["微信小程序<br/>(miniprogram/)"]
    CLOUD -->|"定时检测"| AC["alertChecker"]
    AC --> NT["notify"]
    NT -->|"微信订阅消息推送"| MSG["车辆告警通知"]
```
